...
function App() {
  // ----- elements -----
  const [notes, setNotes] = useState([]); // sticky notes {id,x,y,w,h,color,text}
  const [texts, setTexts] = useState([]); // free text {id,x,y,w,text,size}
  const [shapes, setShapes] = useState([]); // {id,x,y,w,h,type,fill}
  const [stamps, setStamps] = useState([]); // emoji stamps {id,x,y,emoji}

  const [selectedId, setSelectedId] = useState(null); // {type,id}
  const [editingId, setEditingId] = useState(null); // currently text-editing id (note/text)
  ...
  const dragRef = useRef(null); // { kind, id, offsetX, offsetY }
  const resizeRef = useRef(null); // { kind, id, startW, startH, startX, startY, mouseStartX, mouseStartY }
  ...
  const handleBackgroundMouseDown = (e) => {
    // only fire for direct background clicks, not children (children stop propagation)
    ...
    if (selectedTool === "emoji") {
      // emoji tool click just opens menu; placement handled via menu pick
      return;
    }

    // pointer tool clicking empty canvas: deselect
    setSelectedId(null);
    setEditingId(null);
  };
  ...
  const handleBackgroundMouseMove = (e) => {
    ...
    if (dragRef.current) {
      ...
      if (kind === "note") {
        setNotes((arr) => arr.map((n) => (n.id === id ? { ...n, x: nx, y: ny } : n)));
      } else if (kind === "text") {
        setTexts((arr) => arr.map((t) => (t.id === id ? { ...t, x: nx, y: ny } : t)));
      } else if (kind === "shape") {
        setShapes((arr) => arr.map((s) => (s.id === id ? { ...s, x: nx, y: ny } : s)));
      } else if (kind === "stamp") {
        setStamps((arr) => arr.map((s) => (s.id === id ? { ...s, x: nx, y: ny } : s)));
      }
      return;
    }
    if (resizeRef.current) {
      const { kind, id, startW, startH, startX, startY, mouseStartX, mouseStartY } = resizeRef.current;
      const dx = (e.clientX - mouseStartX) / zoom;
      const dy = (e.clientY - mouseStartY) / zoom;
      const newW = Math.max(60, startW + dx);
      const newH = Math.max(60, startH + dy);
      if (kind === "note") {
        setNotes((arr) => arr.map((n) => (n.id === id ? { ...n, w: newW, h: newH } : n)));
      } else if (kind === "shape") {
        setShapes((arr) => arr.map((s) => (s.id === id ? { ...s, w: newW, h: newH } : s)));
      } else if (kind === "text") {
        setTexts((arr) => arr.map((t) => (t.id === id ? { ...t, w: Math.max(80, startW + dx) } : t)));
      }
      // keep x,y anchored at startX/startY (top-left fixed)
      void startX;
      void startY;
    }
  };
  ...
  const startDragElement = (e, kind, id, elX, elY) => {
    if (selectedTool === "hand" || spaceHeld) return; // panning takes priority
    e.stopPropagation();
  ...
  const onToolClick = (tool) => {
    ...
    if (tool === "sticky") {
      // place sticky in current center of viewport
      const rect = containerRef.current.getBoundingClientRect();
  ...
  const pickShapeType = (type) => {
    ...
    setShowShapeMenu(false);
    // tool stays "shapes"; next canvas click places it
  };
    ...
    >
      ...
      >
        ...
        {notes.map((note) => {
          ...
          return (
            ...
            >
              <div
                className={`w-full h-full rounded-2xl shadow-md p-4 flex flex-col ${
                  isSelected ? "ring-2 ring-pink-500 ring-offset-2" : ""
                }`}
              ...
              >
                {isEditing ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      setNotes((arr) =>
                        arr.map((n) => (n.id === note.id ? { ...n, text: e.target.value } : n))
                      )
                    }
                    onMouseDown={(e) => e.stopPropagation()}
                  ...
                  <p className="w-full h-full whitespace-pre-wrap break-words text-gray-800 font-medium text-base leading-snug overflow-hidden">
                    {note.text || (
                      <span className="text-gray-800/40">Double-click to edit</span>
                    )}
                  </p>
                )}
              </div>
                  ...
                  >
                    {STICKY_COLORS.map((c) => (
                      <button
                        ...
                        onClick={() =>
                          setNotes((arr) =>
                            arr.map((n) => (n.id === note.id ? { ...n, color: c } : n))
                          )
                        }
              ...
              >
                {isEditing ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      setTexts((arr) =>
                        arr.map((tt) => (tt.id === t.id ? { ...tt, text: e.target.value } : tt))
                      )
                    }
                    onMouseDown={(e) => e.stopPropagation()}
                    onBlur={() => {
                ...
                {SHAPE_TYPES.map(({ id, label, Icon }) => (
                  <button
                    ...
                    className={`p-2 rounded-xl hover:bg-gray-100 ${
                      pendingShapeType === id ? "bg-pink-100 text-pink-600" : "text-gray-600"
                    }`}
                  >
                    <Icon size={20} />
                  </button>
      ...
      <div className="absolute right-8 bottom-8 z-10 flex items-center gap-3">
        ...
        <div className="bg-white rounded-2xl shadow-md flex overflow-hidden">
          <button
            onClick={() => zoomBy(1.2)}
            className="w-14 h-14 flex items-center justify-center hover:bg-gray-50"
          >
            <Plus />
          </button>
